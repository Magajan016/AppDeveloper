...
const Login = ({addRef,showLogin,setShowLogin}) => {
  ...
  const handleClick = (e) => {
    ...
    else if(!/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(login.email)){
  ...
  return (
    ...
    <div class="fixed w-screen h-screen top-0 flex justify-center z-[100] bg-black bg-opacity-90" on>
      <Toaster/>
      <form
        onSubmit={handleClick}
        ref={addRef}
        class={` relative flex flex-col w-[25rem] h-[80%] top-[5%] m-6 space-y-8 space-x-0  hover:border-yellow-600 bg-white  bg-opacity-100 shadow-2xl rounded-2xl md:flex-row md:space-y-0`} > 
        <div class="flex flex-col justify-center align-items-center p-8 md:p-14">
          ...
          <div class="py-4  justify-between">
            ...
            <input
              ...
              class="w-full p-2 border hover:placeholder:border-yellow-100  hover:border-yellow-100  hover:outline-none hover:bg-yellow-100  rounded-md"
            ></input>
          ...
          <div class="flex flex-col space-y-2">
            ...
            <button  onClick={()=>nav('/home')} class=" flex gap-4 justify-center w-full bg-black  font-serif text-white font-bold p-2 rounded-lg hover:bg-yellow-100  hover:text-black ">
              <FcGoogle class="mt-1" />
              Sign in with Google
            </button>
           ...
           */}
           <label>Doesn't have an account?{" "}
                <span className="hover:underline hover:text-green-500" onClick={()=>nav('/signup')}>Register Now</span></label>
        </div>
      </form>
    </div>
  );
